codeunit 60100 "APO_SalesOrderInject"
{
    ...
    begin
        // Trier les enregistrements par OnlineOrderID
        SalesTemp.SetRange(OriginOrderID);

        if SalesTemp.FindSet() then begin
            LastOrderID := '';

            repeat
                if SalesTemp.OrderID <> LastOrderID then begin
                    if LastOrderID <> '' then
                        SalesHeaderRec.Modify();  // Considérez de supprimer cette ligne si elle n'est pas nécessaire

                    InsertSalesHeader(SalesHeaderRec, SalesTemp);
                    LastOrderID := SalesTemp.OrderID;
                end;

                InsertSalesLine(SalesHeaderRec, SalesLineRec, SalesTemp);

            until SalesTemp.Next() = 0;
